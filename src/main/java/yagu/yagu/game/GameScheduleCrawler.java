...
public class GameScheduleCrawler {
    ...
    private void crawlTableAndSave(WebDriver driver, WebDriverWait wait, int year) {
        ...
        for (WebElement row : rows) {
            if (row.getText().contains("데이터가 없습니다"))
                continue;
    ...
    private void confirmGames() {
        ...
        for (GameRaw raw : parsedGames) {
            try {
                ...
                KboGame game = existing.orElseGet(() -> KboGame.of(
                        raw.getGameDate(),
                        raw.getGameTime(),
                        raw.getHomeTeam(),
                        raw.getAwayTeam()));
    ...
    private LocalTime safeParseTime(String s) {
        if (s == null)
            return null;
        s = s.trim();
        if (s.isEmpty() || "-".equals(s) || s.equalsIgnoreCase("TBD") || s.contains("추후"))
            return null;
        s = s.replaceAll("[^0-9:]", "");
        try {
